...
func NewTransportRateLimiter(c http.RoundTripper) *TransportRateLimiter {
	return &TransportRateLimiter{
		c:   c,
		log: zap.S().With("workerID", utils.RandomString(29)),
	}
}
...
func (b *TransportRateLimiter) roundTripBinance(request *http.Request) (*http.Response, error) {
	...
	if now.Before(blockUntil) {
		b.log.Errorw("rate limit guard, until %s", blockUntil.String())
		return nil, fmt.Errorf("rate limit guard, until %s", blockUntil.String())
	}
	resp, err := b.c.RoundTrip(request)
	if err != nil {
		return resp, err
	}
	...
	b.log.Infow("set binance block until", "to", retryAt.String())
	b.lock.Lock()
...
func (b *TransportRateLimiter) findRetryAt(resp *http.Response, body []byte) time.Time {
	...
	if len(allMatch) == 0 {
		b.log.Errorw("found binance 418 status code, but can't match 'block until'", "body", string(body))
	} else {
...
func (b *TransportRateLimiter) RoundTrip(request *http.Request) (*http.Response, error) {
	if request.URL.Host != "api.binance.com" && request.URL.Host != "fapi.binance.com" {
		return b.c.RoundTrip(request)
	}
